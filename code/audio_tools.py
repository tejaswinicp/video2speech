...
def lpc_synthesis(lp_coefficients, per_frame_gain, residual_excitation=None,
    ...
    if residual_excitation is None:
        ...
        for window in range(n_windows):
            ...
            if voiced_frames[window]:
                ...
                residual_excitation[index.astype(int)] += hanning * sig
            residual_excitation[index.astype(int)] += hanning * 0.01 * random_state.randn(
                int(window_size))
    else:
    ...
    residual_excitation = np.hstack((residual_excitation,
                                     np.zeros(int(window_size))))
    if voiced_frames is None:
        voiced_frames = np.ones_like(per_frame_gain)

    synthesized = np.zeros((int(n_points)))
    for window in range(n_windows):
        window_base = window * window_step
        oldbit = synthesized[int(window_base) + np.arange(int(window_step))]
        ...
        newbit = g_coefs * sg.lfilter([1], w_coefs,
                                      residual_excitation[index.astype(int)])
        synthesized[index.astype(int)] = np.hstack((oldbit, np.zeros(
            (int(window_size) - int(window_step)))))
        synthesized[index.astype(int)] += sg.hanning(int(window_size)) * newbit
