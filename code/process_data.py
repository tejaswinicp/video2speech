...
def process_video(vf, viddata, vidctr, faceCascade):
	...
	for i in np.arange(MARGIN,SAMPLE_LEN+MARGIN):
		viddata[vidctr,:,:,:] = temp_frames[int(COLORS*(i-MARGIN)):int(COLORS*(i+MARGIN)),:,:]
		vidctr = vidctr+1
	return vidctr
...
def process_audio(af, auddata, audctr):
	...
	auddata[int(audctr):int(audctr)+int(SAMPLE_LEN),:] = feat
...
def main():
	...
	if isfile(viddata_path) and isfile(auddata_path):
		...
		viddata = np.zeros((int(MAX_FRAMES),CHANNELS,FRAME_ROWS,FRAME_COLS),dtype="uint8")
		auddata = np.zeros((int(MAX_FRAMES),NET_OUT),dtype="float32")
